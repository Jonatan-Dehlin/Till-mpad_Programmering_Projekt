...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		...
		# Pan med högerklick samtidigt som zoom
...
func get_camera_rect() -> Rect2: # Får kamerans rect
	var size = get_viewport_rect().size / zoom
	var half = size * 0.5
	return Rect2(global_position - half, size)

func clamp_camera_to_limits(): # Ser till att kameran inte sticker utanför skärmen
